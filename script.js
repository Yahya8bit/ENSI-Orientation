...
function closeOpenSpecAcc() {
  const body = [...document.querySelectorAll('.spec-acc-body')].find(b=>b.style.maxHeight && b.style.maxHeight!=='0px');
  if (!body) return false;
  body.style.maxHeight = '0px';
  const item = body.closest('.spec-acc-item');
  const chev = item ? item.querySelector('.spec-acc-chev') : null;
...
window.addEventListener('popstate', function() {
  ...
  if (closeOpenSpecAcc()) { popOverlayHistoryIfNeeded(true); return; }
...
function toggleSpecAcc(btn) {
  const item = btn.parentElement;
  const body = item.querySelector('.spec-acc-body');
  const chev = btn.querySelector('.spec-acc-chev');
  const isOpen = body.style.maxHeight && body.style.maxHeight !== '0px';
  const anyOpenBefore = [...document.querySelectorAll('.spec-acc-body')].some(b=>b.style.maxHeight && b.style.maxHeight!=='0px');
  if (!isOpen) {
    _accOpen(document.querySelectorAll('.spec-acc-body'), document.querySelectorAll('.spec-acc-chev'), item, body, chev);
    if (!anyOpenBefore) pushOverlayHistory();
  } else {
    body.style.transition='none'; body.style.maxHeight='0px';
    chev.style.transform='';
...
async function init() {
  ...
  const updateScrollProgress = () => {
    ...
    if (track) {
      ...
      track.style.setProperty('--scroll-progress', (max > 0 ? (window.scrollY / max) * 100 : 0) + '%');
    }
  };
...
// ─── SKELETON (shown synchronously before CLUBS fetch resolves) ──────────────
function skeletonClubTile() {
  return `<div>
    <div class="skeleton" style="width:100%;aspect-ratio:1/1;border-radius:8px;margin-bottom:10px"></div>
    <div class="skeleton" style="width:70%;height:16px;border-radius:4px;margin-bottom:6px"></div>
    <div class="skeleton" style="width:45%;height:12px;border-radius:4px"></div>
  </div>`;
}

function renderSkeleton() {
  document.getElementById('page').innerHTML = `
  <section class="hero-section" id="hero-section" style="background:#1B2A4A"></section>

  <section id="section-numbers" style="background:#1B2A4A;padding:80px 10vw">
    <div class="skeleton" style="width:220px;height:38px;border-radius:6px;margin-bottom:40px"></div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:24px">
      ${Array(4).fill(0).map(()=>'<div class="skeleton" style="height:120px;border-radius:12px"></div>').join('')}
    </div>
  </section>

  <section id="section-admission" style="background:#fff;padding:96px 10vw">
    <div class="skeleton" style="width:180px;height:18px;border-radius:4px;margin-bottom:16px"></div>
    <div class="skeleton" style="width:420px;max-width:100%;height:48px;border-radius:6px;margin-bottom:40px"></div>
    <div class="skeleton" style="width:100%;height:220px;border-radius:8px"></div>
  </section>

  <section style="background:#F5F7FA;padding:96px 10vw">
    <div class="skeleton" style="width:180px;height:18px;border-radius:4px;margin-bottom:16px"></div>
    <div class="skeleton" style="width:420px;max-width:100%;height:48px;border-radius:6px;margin-bottom:40px"></div>
    <div style="display:flex;flex-direction:column;gap:26px">
      ${Array(5).fill(0).map(()=>'<div class="skeleton" style="height:34px;border-radius:4px"></div>').join('')}
    </div>
  </section>

  <section id="section-studentlife" style="background:#fff;padding:96px 10vw">
    <div class="skeleton" style="width:220px;height:18px;border-radius:4px;margin-bottom:12px"></div>
    <div class="skeleton" style="width:280px;height:48px;border-radius:6px;margin-bottom:40px"></div>
    <div id="club-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px">
      ${Array(8).fill(0).map(skeletonClubTile).join('')}
    </div>
  </section>

  <section id="curriculum-section" style="background:#fff;padding:80px 10vw">
    <div class="skeleton" style="width:180px;height:18px;border-radius:4px;margin-bottom:16px"></div>
    <div class="skeleton" style="width:520px;max-width:100%;height:48px;border-radius:6px;margin-bottom:28px"></div>
  ...
  <section id="section-pathways" style="background:#fff;padding:80px 0">
    <div id="pathways-header" style="padding:0 10vw;margin-bottom:40px">
      <div class="skeleton" style="width:260px;height:18px;border-radius:4px;margin-bottom:16px"></div>
      <div class="skeleton" style="width:420px;max-width:100%;height:48px;border-radius:6px"></div>
    </div>
    <div style="border-top:1px solid #E8EDF4">
      ${Array(6).fill(0).map(()=>'<div style="padding:20px 10vw"><div class="skeleton" style="height:56px;border-radius:8px"></div></div>').join('')}
    </div>
  </section>`;
}
...
function renderHome() {
  ...
  // Year bracket spans for timeline
  // S1–S2 = Year 1, S3–S4 = Year 2, S5 = Year 3
  // We render via a flex row with relative widths
  ...
  <section id="section-numbers" style="background:#fff;padding:96px 10vw">
    ...
    <div id="numbers-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:40px;margin-bottom:64px">
      <div class="reveal">
        <div class="stat-num" data-target="2500" data-suffix="+" style="font-family:'Archivo',sans-serif;font-size:3.5rem;font-weight:800;color:#1B2A4A;line-height:1;letter-spacing:-0.03em">0</div>
      ...
      <div class="reveal">
        <div class="stat-num" data-target="220" data-prefix="~" style="font-family:'Archivo',sans-serif;font-size:3.5rem;font-weight:800;color:#1B2A4A;line-height:1;letter-spacing:-0.03em">~0</div>
        <div style="font-family:'Inter',sans-serif;font-size:0.875rem;color:#6B7280;margin-top:8px">nouveaux diplômés chaque année</div>
      </div>
      <div class="reveal">
        <div class="stat-num" data-target="98" style="font-family:'Archivo',sans-serif;font-size:3.5rem;font-weight:800;color:#1B2A4A;line-height:1;letter-spacing:-0.03em">0</div>
      ...
      <div class="reveal">
        <div class="stat-num" data-target="1984" data-plain="1" style="font-family:'Archivo',sans-serif;font-size:3.5rem;font-weight:800;color:#1B2A4A;line-height:1;letter-spacing:-0.03em">0</div>
      ...
      <table id="admission-table" style="width:100%;border-collapse:collapse;min-width:480px">
        ...
        <tbody>
          ...
          <tr style="border-bottom:1px solid rgba(27,42,74,0.08)">
            ...
            <td class="rank-num" data-target="${r2025}" style="padding:22px 32px 22px 0;vertical-align:middle;font-family:'JetBrains Mono',monospace;font-size:1.75rem;font-weight:600;color:#1B2A4A">0</td>
            <td class="col-2024 rank-num" data-target="${r2024}" style="padding:22px 0 22px 0;vertical-align:middle;font-family:'JetBrains Mono',monospace;font-size:1.75rem;font-weight:600;color:#1B2A4A">0</td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>
  ...
  <section id="section-studentlife" style="background:#fff;padding:96px 10vw">
    ...
    <div id="club-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px">
      ${CLUBS.map(c=>`
      <button class="club-tile reveal" data-cat="${c.category}" data-id="${c.id}" onclick="openClubModal('${c.id}')" aria-label="Voir les détails de ${c.name}" style="-webkit-tap-highlight-color:${c.accent}33">
  ...
  <!-- SPECIALIZATIONS STRIPS (PATHWAYS) -->
  <section id="section-pathways" style="background:#fff;padding:80px 0">
    <div id="pathways-header" style="padding:0 10vw;margin-bottom:40px">
    ...
    <div style="border-top:1px solid #E8EDF4">
      ${SPECIALIZATIONS.map(sp=>{
        ...
        return `
      <a href="#spec-${sp.id}" class="spec-strip reveal" style="--sp-color:${sp.color}" onclick="event.preventDefault();const el=document.getElementById('spec-${sp.id}');if(el){const hdr=el.querySelector('.spec-acc-hdr');if(hdr)toggleSpecAcc(hdr);const nav=document.getElementById('navbar');const offset=nav?nav.offsetHeight+16:80;window.scrollTo({top:el.getBoundingClientRect().top+window.scrollY-offset,behavior:'smooth'})}">
...
function specializationsSection() {
  ...
  return `
  <!-- COMMON S5 MODULES -->
  <section style="background:#F5F7FA;padding:64px 10vw">
    <h2 style="font-family:'Archivo',sans-serif;font-size:1rem;font-weight:600;color:#1B2A4A;margin:0 0 16px;letter-spacing:0.05em;text-transform:uppercase;font-size:0.8rem;color:#4DB8E8">Modules S5 communs (toutes les filières)</h2>
  ...
  <section style="padding:64px 10vw">
    <div style="display:flex;flex-direction:column;gap:28px">
      ${SPECIALIZATIONS.map(sp=>{
        ...
        const modGrid = (mods, accent) => mods.length ? `
        ...
        return `
        <div id="spec-${sp.id}" class="spec-acc-item reveal" style="background:#fff;border:1px solid #E8EDF4;border-radius:14px;overflow:hidden;width:100%;max-width:100%;box-sizing:border-box">
          <button class="spec-acc-hdr" onclick="toggleSpecAcc(this)" style="width:100%;display:flex;align-items:center;gap:16px;padding:20px 28px;background:${sp.color}06;border:none;border-bottom:1px solid ${sp.color}25;cursor:pointer;text-align:left">
            <div class="spec-acc-icon" style="font-family:'JetBrains Mono',monospace;font-size:1.5rem;font-weight:600;color:${sp.color};background:${sp.color}15;padding:10px 14px;border-radius:8px;flex-shrink:0">${SPEC_BADGE_LABEL[sp.id]||sp.id}</div>
            <span class="spec-acc-title" style="font-family:'Archivo',sans-serif;font-size:1.1rem;font-weight:800;color:#1B2A4A;flex:1;min-width:0">${sp.nameFr}</span>
            <span class="spec-acc-chev" style="color:${sp.color};font-size:1.3rem;font-weight:700;transition:transform .25s ease;display:inline-block;flex-shrink:0">›</span>
          </button>
          <div class="spec-acc-body" style="max-height:0;overflow:hidden;transition:max-height .3s ease">
            <div class="spec-header-row" style="padding:24px 28px;border-bottom:1px solid ${sp.color}25;background:${sp.color}06;display:flex;align-items:flex-start;gap:16px">
              <div class="spec-header-text">
                <p style="font-size:0.8rem;color:#9CA3AF;font-style:italic;margin:0 0 8px">${sp.name}</p>
                <p style="font-size:0.875rem;color:#4B5563;margin:0">${sp.desc}</p>
              </div>
            </div>
            <div style="padding:20px 28px;display:flex;flex-direction:column;gap:20px;width:100%;max-width:100%;box-sizing:border-box;overflow:hidden">
              ${spComp.length ? `<div style="width:100%;max-width:100%;box-sizing:border-box">
                ...
                ${modGrid(spComp, sp.color)}
              </div>` : ''}
              ${spOpt.length ? `<div style="width:100%;max-width:100%;box-sizing:border-box">
...
function observeReveal(selector, onReveal, options={threshold:0.3}) {
...
function countUpElement(el, duration=1200) {
  ...
  const step = (now) => {
    ...
    el.textContent = prefix + (el.dataset.plain ? val : formatFR(val)) + suffix;
...
function initDegreeBars() {
  observeReveal('.degree-bar-row', el => {
    const fill = el.querySelector('.degree-bar-fill');
    fill.style.width = el.dataset.pct + '%';
  });
}

function initStatsReveal() {
  observeReveal('#section-numbers .reveal', el => {
    el.classList.add('in');
    const num = el.querySelector('.stat-num');
    if (num) countUpElement(num);
  });
}

function initAdmissionReveal() {
  observeReveal('#admission-table tbody tr', el => {
    el.querySelectorAll('.rank-num').forEach(td => countUpElement(td));
  });
}

function initEntranceReveal() {
  observeReveal('#club-grid .reveal, .spec-strip.reveal, .spec-acc-item.reveal', el => el.classList.add('in'));
}
    ...
    <div id="footer-grid" style="display:grid;grid-template-columns:1fr auto auto;gap:48px;align-items:start;flex-wrap:wrap">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:10px">
          <a href="#home" style="${linkStyle}" onclick="event.preventDefault();if(location.hash==='#home'||location.hash===''){window.scrollTo({top:0,behavior:'smooth'})}else{navigate('home')}" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='rgba(255,255,255,0.5)'">Accueil</a>
          <a href="#home" style="${linkStyle}" onclick="event.preventDefault();navigate('home');setTimeout(()=>{const el=document.getElementById('section-pathways');if(el)el.scrollIntoView({behavior:'smooth'})},50)" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='rgba(255,255,255,0.5)'">Filières</a>
